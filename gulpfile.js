...
const { template, split, slice, sortBy, join } = require('lodash');
const through = require('through2');
const i18nextScanner = require('i18next-scanner');

const i18nextScannerConfig = require('./i18next-scanner.config');

gulp.task('i18next-scanner', () =>
  gulp.src(i18nextScannerConfig.input).pipe(i18nextScanner(i18nextScannerConfig.options)).pipe(gulp.dest('./'))
);

gulp.task('translate-scanner', gulp.parallel('i18next-scanner'));
  ...
  return gulp
    ...
    .pipe(
      through.obj(function (file, _enc, cb) {
        ...
        const tmpl = fs.readFileSync('./docs/todos.template.md', 'utf8');
        ...
        // change file name
        file.path = path.join(file.base, './docs/todos.md');
